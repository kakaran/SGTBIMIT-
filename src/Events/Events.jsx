...
export default function Events() {
    ...
    useEffect(() => {
      const EventDataGet = async () =>{
        try {
            const data = (await axios.get(`${API_URL}/Eventhandler/EventHandler_Single_Display/${id}`)).data
            // console.log(data);
            setEvents(data);
        } catch (error) {
            console.log(error);
    ...
    // console.log(event?.Years[0]?.year);

    // const [eventFilter,setEventFilter] = useState(events?.Years[0]?.year);
    // setEventFilter(events?.Years[0]?.year)
        ...
        <>
            ...
            {events && <section className='event-section'>
                <div className="event-header">
                    ...
                    <img src={`${API_URL}/Eventhandler/EventHandleR_Heder_Image/${events._id}`} alt="" />
                    <div className="event-header-title">
                        <p>Home {">"} Societies {">"} {events.name}</p>
                        <h1> {events.name} </h1>
                    </div>
                </div>
                    ...
                    <div className="about-event-carousel">
                        ...
                        >
                            {events.images.map((image) => (
                                <img src={`${API_URL}/Eventhandler/EventHandler_Image_Display/${events._id}/${image._id}`} alt="cant load" className='event-carousel-img' />
                            ))}
                        </Carousel>
                    </div>
                </div>
