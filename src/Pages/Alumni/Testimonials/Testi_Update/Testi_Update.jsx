...
const AlumniTesti_Update = () => {
  ...
  useEffect(() => {
    const TestSingleData = async () => {
      try {
        const data = (
          await axios.get(
            `${import.meta.env.VITE_API_URL}/alumini_Testimonial/alumini_Testimonial_Display/${_id}`
          )
        ).data;
        setTestiUpdate({
          name: data?.name,
  ...
  const AlumniTestiUpdate = async () => {
    try {
      ...
      const data1 = (
        await axios.post(
          `${import.meta.env.VITE_API_URL}/alumini_Testimonial/alumini_Testimonial_Update/${_id}`,
          formData,
          {
            headers: {
            ...
            <div className="SocietyForm">
              ...
              <div className="Message_image">
                ...
                {filedata ? (
                  ...
                  <img
                    src={`${import.meta.env.VITE_API_URL}/alumini_Testimonial/alumini_Testimonial_Image_Display/${_id}`}
                    alt=""
                    style={{
                      width: "400px",
                      height: "200px",
