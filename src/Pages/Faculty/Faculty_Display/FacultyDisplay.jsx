...
const FacultyDisplay = () => {
  ...
  useEffect(() => {
    const TestimonialsDataGet = async () => {
      try {
        const data = (
          await axios.get(`${import.meta.env.VITE_API_URL}/Faculty/Faculty_Display`)
        ).data;
        setFaculty(data);
        setRender(0);
      } catch (error) {
  ...
  const ImagesGet = (value) => {
    return (
      <img
        src={`${import.meta.env.VITE_API_URL}/Faculty/Faculty_Image_Display/${value}`}
        alt=""
  ...
  const FacultyDelete = async (value) => {
    try {
      const _id = value;
      // console.log(_id);
      await axios.delete(`${import.meta.env.VITE_API_URL}/Faculty/Faculty_Delete/${_id}`);
      setRender(1);
    } catch (error) {
      console.log(error);
    }
