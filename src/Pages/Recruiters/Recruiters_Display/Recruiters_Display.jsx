...
const RecruitersDisplay = () => {
  ...
  useEffect(() => {
    const TestimonialsDataGet = async () => {
      try {
        const data = (
          await axios.get(`${import.meta.env.VITE_API_URL}/Recruiters/recruiters_Display`)
        ).data;
        setRecruiters(data);
        setRender(0);
      } catch (error) {
  ...
  const ImagesGet = (value) => {
    return (
      <img
        src={`${import.meta.env.VITE_API_URL}/Recruiters/Recruiter_Image_Display/${value}`}
        alt=""
  ...
  const RecruitersDelete = async (value) => {
    try {
      const _id = value;
      // console.log(_id);
      await axios.post(`${import.meta.env.VITE_API_URL}/Recruiters/recruiters_Delete/${_id}`);
      setRender(1);
    } catch (error) {
      console.log(error);
    }
